...
Puppet::Type.type(:ext_ssh_authorized_key).provide(
  :parsed,
  ...
  record_line :parsed,
    :fields   => %w{options type key comment},
    :optional => %w{options},
    :rts => /^\s+/,
    ...
    :post_parse => proc { |h|
      ...
      h[:options] ||= [:absent]
      h[:options] = Puppet::Type::Ext_ssh_authorized_key::ProviderParsed.parse_options(h[:options]) if h[:options].is_a? String
    },
    :pre_gen => proc { |h|
  ...
  record_line :key_v1,
    :fields   => %w{options bits exponent modulus comment},
    :optional => %w{options},
    :rts      => /^\s+/,
    :match    => /^(?:(.+) )?(\d+) (\d+) (\d+)(?: (.+))?$/
  ...
  def self.prefetch(resources)
    ...
    @records.each do |record|
      ...
      resource = nil
      resources.each do |resource_name, hash|
        if record[:type] == hash[:type].to_sym.to_s and record[:comment] == hash[:comment] and record[:target] == hash[:target]
            resource = hash
            break
        end
      end
